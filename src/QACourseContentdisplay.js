...
import "./css/QACourseContentdisplay.css";
    ...
    fetch("https://sxt7404.uta.cloud/php/qacoursedetails.php")
      ...
      .then((data) => {
        ...
        const uniquePrograms = Array.from(
          new Set(data.map((program) => program.program_name))
        ).map((program_name) => {
          return {
            program_name: program_name,
            courses: data.filter(
              (course) => course.program_name === program_name
            ),
          };
        });
            ...
            <li key={program.program_name} className="program-item">
              <button
                className={`program-button ${
                  selectedProgram === program.program_name ? "active" : ""
                }`}
                    ...
                    <li key={course.course_name} className="course-item">
                      <button
                        className={`course-button ${
                          selectedCourse === course.course_name ? "active" : ""
                        }`}
                      ...
                      {selectedCourse === course.course_name && (
                        <div className="module-details">
                          <p>
                            ...
                            {course.courseModuleName || "N/A"}
                          </p>
                          <p>
                            <strong>Module Content:</strong>{" "}
                            {course.courseModuleContent || "N/A"}
                          </p>
                        </div>
                      )}
                    </li>
