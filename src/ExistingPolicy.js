...
function ExistingPolicies() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost/existing_policy.php')
  ...
  const handleEditClick = (policy) => {
    ...
    const handlePolicyChange = (policy, field, value) => {
      ...
      const handleSaveClick = (policy) => {
        ...
        fetch('http://localhost/update_policy.php', {
          method: 'POST',
          headers: {
      ...
      const handleDeleteClick = (policy) => {
        ...
        const confirmDelete = window.confirm('Press OK to delete or Cancel to keep.');
        ...
        if (confirmDelete) {
          ...
          fetch(`http://localhost/delete_policy.php?id=${policy.id}`)
            .then((response) => response.text())
            .then((responseText) => {
        ...
        <tbody className='expolicytbody'>
          {policies.map((policy) => (
            <tr className='expolicytr' key={policy.id}>
              <td className='expolicytd'>{editablePolicy === policy ? (
                <input
                  type="text"
                  value={policy.name}
                  onChange={(e) => handlePolicyChange(policy, 'name', e.target.value)}
                />
              ) : (
                policy.name
              )}</td>
              <td className='expolicytd'>{editablePolicy === policy ? (
                <input
                  ...
                  onChange={(e) => handlePolicyChange(policy, 'description', e.target.value)}
                />
              ) : (
                policy.description
              )}</td>
              <td className='expolicytd'>{editablePolicy === policy ? (
                <input
                  type="number"
                  value={policy.amount}
                  onChange={(e) => handlePolicyChange(policy, 'amount', e.target.value)}
                />
              ) : (
                policy.amount
              )}</td>
              <td className='expolicytd'>{editablePolicy === policy ? (
                <input
                  ...
                  onChange={(e) => handlePolicyChange(policy, 'start_date', e.target.value)}
                />
              ) : (
                policy.start_date
              )}</td>
              <td className='expolicytd'>{editablePolicy === policy ? (
                <input
                  ...
                  onChange={(e) => handlePolicyChange(policy, 'expiration_date', e.target.value)}
                />
              ) : (
              ...
              <div className='button-container'>
                {editablePolicy === policy ? (
                    <>
                    <button className="savepolicybutton" onClick={() => handleSaveClick(policy)}>Save</button>
                    </>
                ) : (
                    <>
                    <button className="editpolicybutton" onClick={() => handleEditClick(policy)}>Edit</button>
                    <button className='delete-button' onClick={() => handleDeleteClick(policy)}>Delete</button>
                    </>
                )}
              </div>
              </td>
