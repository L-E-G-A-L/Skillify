...
function InquiryInbox() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://sxt7404.uta.cloud/php/getEnquiries.php"
        );
  ...
  const markAsRead = async (inquiryId) => {
    try {
      await axios.put("https://sxt7404.uta.cloud/php/getEnquiries.php", {
        enquiryData_id: inquiryId,
      });
            ...
            {inquiries.map((inquiry) => (
              <li key={inquiry.enquiryData_id} className="inquiry-item">
                ...
                {parseInt(inquiry.isRead, 10) === 0 ? (
                  <button
                    onClick={() => markAsRead(inquiry.enquiryData_id)}
